...
Model formula notation is familiar to R: `architect` was designed with the idea formulaic model specification in mind.
...
Underneath the hood, the libraries `data.table`, `rlang`, and `purrr` are combined to allow for the above to be expressed as a series of definitions of how to modify a dataset: $$
D_{i+1} \sim g\big(D_{i}\big) := g\big(D_i \Rightarrow f(\Lambda_m)\sim X_n\big)
$$
